<?php 
    ...
    function getservices($lang){
        if ($lang == 'en') {
            $query = "select services_id, services_name_en as name, services_descript_en as descript from services";
        }
        else {
            $query = "select services_id, services_name as name, services_descript as descript from services";
        }
        return mysql_query($query);
    }
    function getchuyenkhoa_byID($service_id, $lang){
        if($lang == 'en'){
            $query = "SELECT ck.chuyenkhoa_id, ck.chuyenkhoa_name_en as name, ck.chuyenkhoa_descript_en as descript FROM chuyenkhoa AS ck, services AS serv WHERE ck.services_id = serv.services_id and serv.services_id = $service_id";
        }
        else {
            $query = "SELECT ck.chuyenkhoa_id, ck.chuyenkhoa_name as name, ck.chuyenkhoa_descript as descript FROM chuyenkhoa AS ck, services AS serv WHERE ck.services_id = serv.services_id and serv.services_id = $service_id";
        }
        return mysql_query($query);
    }
