$(function(){
  ...
  $('#autocomplete').autocomplete({
        ...
        onSelect: function (suggestion) {
            ...
            // }, 1000);
            if(floor == $('#imageMap0').attr("data-id")){
                var img = $('#imageMap0');
                img.mapster('set',null,suggestion.data);
                img.mapster('tooltip',suggestion.data);
            }
            else {
                $('#imageMap0').attr("data-id",floor);
                $('#imageMap0').attr("usemap","#map"+floor);
                $('#imageMap0').attr("src","../images/floor/tang"+floor+".jpg");
                $('.mapster_el').attr("src","../images/floor/tang"+floor+".jpg");
                $('#grid-load').html("<img src='../admin/upload/image_doctor/loading.gif'/>").fadeIn('fast');
                $('#imageMap0').load(function(){
